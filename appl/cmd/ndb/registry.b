...
Qfind,
Qsvc:	con iota;
Never:	con 0;	# for Service.expire
...
Service: adt {
	...
	atime:	int;
	mtime:	int;
	expire:	int;		# expires if now == expire
	vers:		int;
...
expire(now: int)
{
	if(now == lastchecked)
		return;
	lastchecked = now;
	l: list of ref Service;
	l = nil;
	for(i := 0; i < nservices; i++)
		if(services[i].expire != Never && services[i].expire <= now)
			l = services[i]::l;
	if(l != nil){
		for(; l != nil; l = tl l)
			(hd l).remove();
